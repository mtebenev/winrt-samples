...
namespace Mt.Common.UiCore.Controls
{
	public partial class ColorHexagonPicker : ColorBaseControl
	{
		...
		protected internal override void UpdateLayoutBasedOnColor()
		{
			...
			Rectangle rect = _rectangles.First(r => ((SolidColorBrush)r.Fill).Color == Color);
		...
		public void GenerateLayout()
		{
			...
			int totalSteps = ColorBrightnessSteps + ColorDarknessSteps;
			...
			if(totalSteps > 0)
			{
				StackPanel colorHexBody = new StackPanel();
			...
			if(GreyScaleSteps > 0)
			{
				StackPanel greyScaleHexBody = new StackPanel();
				greyScaleHexBody.Margin = new Thickness(0, ColorSize, 0, 0);

				#region building grey scale grid

				StackPanel topGrey = CreateHorizontalStackPanel();
				StackPanel bottomGrey = CreateHorizontalStackPanel();

				int totalGreySteps = GreyScaleSteps + 2; // including pure white and pure black
				double greyStep = 255 / (double)totalGreySteps;

				for(int i = 0; i <= totalGreySteps; i++)
				{
					byte step = (byte)(greyStep * i);
					Rectangle rect = CreateRectangle(Color.FromArgb(255, step, step, step));
		...
		private static double CalculateAngle(double x1, double y1)
		{
			...
			double angle = Math.Atan2(y1, x1) * (180 / Math.PI);
		...
		{
			ColorHexagonPicker sender = o as ColorHexagonPicker;
