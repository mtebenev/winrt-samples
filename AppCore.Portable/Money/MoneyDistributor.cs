namespace System
{
	public class MoneyDistributor
	{
		...
		public Money[] Distribute(decimal distribution)
		{
			...
			int distributionCount = (Int32) Math.Floor(1/distribution);
			...
			{
				Money toDistribute = _toDistribute;
				Money part = toDistribute/distributionCount;
			...
			switch(_receiver)
			{
				case FractionReceivers.FirstToLast:
					for(int i = 0; i < remainder/quantum; i++)
					{
						result[i] += quantum;
				...
				case FractionReceivers.LastToFirst:
					for(int i = (Int32) (remainder/quantum); i > 0; i--)
					{
						result[i] += quantum;
						_distributedTotal += quantum;
					}
