...
using Mt.Common.WinRtAppCore.Platform;
...
namespace System
{
	...
	public struct Currency : IEquatable<Currency>, IFormatProvider
	{
		...
		private static readonly Dictionary<string, Int32> _lcidToIsoCodeLookup
			= new Dictionary<string, Int32>();

		private static readonly Dictionary<Int32, List<string>> _isoCodeToLcidLookup
		...
		static Currency()
		{
			IReadOnlyCollection<CultureInfo> cultures = CultureUtility.GetCultures(CultureUtility.CultureTypes.SpecificCultures);

			Dictionary<string, List<string>> cultureIdLookup = new Dictionary<String, List<string>>();
			Dictionary<string, string> symbolLookup = new Dictionary<String, String>();

			foreach(CultureInfo culture in cultures)
			{
				string lcid = culture.Name;
				...
				{
					cultureIdLookup[isoSymbol] = new List<string>();
				}
		...
		public static Currency FromCultureInfo(CultureInfo cultureInfo)
		{
			...
			if(_lcidToIsoCodeLookup.TryGetValue(cultureInfo.Name, out currencyId))
			{
		...
		public static Boolean TryParse(String s, out Currency currency)
		{
			...
			{
				if(_lcidToIsoCodeLookup.TryGetValue(CultureInfo.CurrentCulture.Name, out currencyId) &&
		...
		public Object GetFormat(Type formatType)
		{
			...
			List<string> lcids = _isoCodeToLcidLookup[_id];

			string lcid = lcids.Contains(CultureInfo.CurrentCulture.Name)
