...
namespace System
{
	                      ...
	                      IComparable
	{
		...
		public static Money operator +(Money left, Money right)
		{
			...
			int fractionSum = left._decimalFraction + right._decimalFraction;

			long overflow = 0L;
			int fractionSign = Math.Sign(fractionSum);
			int absFractionSum = Math.Abs(fractionSum);
			...
			long newUnits = left._units + right._units + overflow;
		...
		public Money Round(RoundingPlaces places, MidpointRoundingRule rounding, out Money remainder)
		{
			...
			int placesExponent = getExponentFromPlaces(places);
			int fraction = roundFraction(placesExponent, rounding, out unit);
			long units = _units + unit;
		...
		{
			decimal denominator = FractionScale/(Decimal) Math.Pow(10, exponent);
			decimal fraction = _decimalFraction/denominator;
			...
			switch(rounding)
			{
				...
				case MidpointRoundingRule.Stochastic:
					...
					double coinFlip = _rng.NextDouble();
